...
`$CLAUDE_PLUGIN_ROOT` is **empty in your (orchestrator) shell** — Claude Code substitutes it
in the `allowed-tools` frontmatter above and sets it inside the plugin's own hooks, but it is
not exported to the Bash calls you issue from this command body, so a literal
`python3 "$CLAUDE_PLUGIN_ROOT"/scripts/…` runs as `/scripts/…` and errors. Resolve the install
path once and use that absolute path in every invocation below:
...
self-hosting inside the plugin repo itself, the repo copy `plugins/fairmind-coding/scripts/…`
also works.)
...
Resolve the plugin install path (see "Resolving the script path" above) — call it PLUGIN_ROOT.

Run the SonarQube analysis script to detect PR and fetch issues:
Execute: `python3 "$PLUGIN_ROOT"/scripts/analyze_sonarqube.py` (with PLUGIN_ROOT expanded to the resolved absolute path)
